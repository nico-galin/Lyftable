...
const ActiveWorkoutPage = ({ route }) => {
  const { userWorkouts, addUserWorkout, replaceUserWorkout, openModal } =
    useAppContext();
  const { activeWorkoutID, activateWorkout, deactivateWorkout } =
  ...
  const [exerciseTimers, setExerciseTimers] = useState(
    exercises.map(ex => {
      if (ex.elapsed_time) {
        return ex.elapsed_time;
      }
      return 0;
    }),
  );
  const [exerciseIntervals, setExerciseIntervals] = useState(
    ...
    const syncInterval = setInterval(() => {
      replaceUserWorkout(
        generateWorkoutObject(
          stateRef.current.workoutMetadata,
    ...
    try {
      if (activeWorkoutID) {
        ...
        const timeDifference = differenceInMilliseconds(
          new Date(),
          parseISO(activeWorkout.start_time),
        );
        // There is an active workout currently
        setWorkoutMetadata({
          id: activeWorkout.id,
          start_time:
            activeWorkout.start_time != null
        ...
        setExerciseTimers(
          activeWorkout.split.exercises.map(ex =>
            ex.elapsed_time != null ? ex.elapsed_time : 0,
          ),
        );
        ...
        const newWorkoutMetadata = {
          id:
            route.params && route.params.data && route.params.data.id != null
              ? route.params.data.id
              : generateUniqueId(),
          start_time:
            route.params &&
            route.params.data &&
            route.params.data.start_time != null
              ? route.params.data.start_time
              : new Date().toISOString(),
          scheduled:
            route.params && route.params.data && route.params.data.scheduled
              ? route.params.data.scheduled
              : null,
          completed:
            route.params &&
            route.params.data &&
            route.params.data.completed != null
              ? route.params.data.completed
              : false,
        };
          ...
          newExercises = route.params.data.split.exercises.map((ex, ind) => {
            return {
              ...
              weights:
                ex.weights != null
                  ? ex.weights
                  : new Array(ex.repetitions.length),
              previous_weights:
                ex.previous_weights != null
                  ? ex.previous_weights
                  : new Array(
                      ex.repetitions.length,
                    ) /* [INCOMPLETE] Need to set prev weights*/,
              previous_repetitions:
                ex.previous_repetitions != null
                  ? ex.previous_repetitions
                  : new Array(
                      ex.repetitions.length,
                    ) /* [INCOMPLETE] Need to set prev reps*/,
              completion_times:
                ex.completion_times != null
                  ? ex.completion_times
                  : new Array(ex.repetitions.length),
              completed: ex.completed != null ? ex.completed : false,
            };
          });
          newExerciseTimers = route.params.data.split.exercises.map(ex =>
            ex.elapsed_time ? ex.elapsed_time : 0,
          );
  ...
  const activateExercise = ind => {
    setExerciseIntervals(oldExerciseIntervals => {
      let newExerciseIntervals = oldExerciseIntervals.map(exInterval =>
        JSON.parse(JSON.stringify(exInterval)),
      );
      newExerciseIntervals[ind] = setInterval(() => {
        setExerciseTimers(oldExerciseTimers => {
          let newExerciseTimers = [...oldExerciseTimers];
    ...
    setExercises(prevExercises =>
      prevExercises.map((ex, ind) => {
        ...
        return {
          ...
          repetitions: ex.repetitions.concat([
            ex.repetitions[ex.repetitions.length - 1],
          ]),
      ...
      prevExercises.map((ex, ind) => {
        ...
        return {
          ...
          repetitions: ex.repetitions
            .slice(0, splitInd)
            .concat(ex.repetitions.slice(splitInd + 1)),
          weights: ex.weights
            .slice(0, splitInd)
            .concat(ex.weights.slice(splitInd + 1)),
        };
      }),
    );
  };
    ...
    openModal('AddExercise', ex => {
      setExercises(prevExercises => {
        ...
        newExercises.push({
          ...ex,
          weights:
            ex.weights != null ? ex.weights : new Array(ex.repetitions.length),
          previous_weights:
            ex.previous_weights != null
              ? ex.previous_weights
              : new Array(
  ...
  const toggleExerciseTimers = timersOn => {
    if (!timersOn) {
      ...
      setExerciseIntervals(
        oldExerciseIntervals => new Array(oldExerciseIntervals.length),
      );
      setExercises(oldExercises => {
    ...
    if (ex.open) {
      ...
      return (
        <TouchableHighlight
          ...
          underlayColor={
            ex.completed
              ? theme.SECONDARY_COLOR_FADED_SELECTED
              : theme.CALENDAR_HIGHLIGHT_COLOR
          }
          key={ind}
          onPress={() => toggleExercise(ind)}
          style={[
            styles.exerciseClosedContainer,
            ex.completed ? styles.exerciseFinished : styles.exerciseUnfinished,
          ]}>
          <>
            <View style={styles.exerciseLeftContent}>
              ...
              <Text
                ...
                {formatSetsReps(ex.set_count, ex.repetitions)}
              </Text>
            </View>
  ...
  return (
    <View style={systemStyles.pageMarginlessContainer}>
      <View style={systemStyles.pageContainer}>
        <Header
          title={splitMetadata.name}
          subtitle={msToDigital(mainTimer)}
          maxTitleLength={15}
          leftButtonName={'dots-horizontal'}
          leftButtonOnPress={handleOpenMenu}
          leftButtonColor={theme.SUBTITLE_COLOR}
