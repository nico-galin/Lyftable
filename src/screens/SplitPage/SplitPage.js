...
export const SplitPage = ({ route }) => {
  ...
  let [split, setSplit] = useState(
    userSplits[route.params.data.id]
      ? userSplits[route.params.data.id]
      : route.params.data,
  );
  ...
  publicSubscribers.slice(0, 5).forEach((sub, ind) => {
    subscriberList +=
      ind === Math.min(publicSubscribers.length - 1, 4)
        ? sub.name
        : `${sub.name}, `;
  });
  if (publicSubscribers.length === 0) {
    ...
    subscriberList += `, and ${
      Math.max(publicSubscribers.length - 5, 0) +
      split.subscribers.length -
      publicSubscribers.length
    } others`;
  }
    ...
    <View style={systemStyles.pageContainer}>
      {inCollection ? (
        <Header
          ...
          rightButtonOnPress={handleDelete}
        />
      ) : (
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sectionContainer}>
          <View style={styles.sectionDefiner}>
            <MatIcon name={'person'} size={26} color={theme.SUBTITLE_COLOR} />
          </View>
        ...
        <View style={styles.sectionContainer}>
          <View style={styles.sectionDefiner}>
            <MatIcon name={'timer'} size={26} color={theme.SUBTITLE_COLOR} />
          </View>
          <View style={styles.sectionInfo}>
            <Text style={styles.sectionHeader}>Estimated Time</Text>
            <Text style={styles.sectionSubheader}>
              {msToHM(split.estimated_time, 'HM')}
            </Text>
          </View>
        </View>
        <View style={styles.sectionContainer}>
          ...
          <View style={styles.sectionInfo}>
            ...
            {split.exercises.map((exercise, ind) => (
              <View key={ind} style={styles.subsectionContainer}>
                ...
                <Text>
                  {msToHM(exercise.time_limit)}{' '}
                  {formatSetsReps(exercise.set_count, exercise.repetitions)}
                </Text>
        ...
        <View style={[styles.sectionContainer, styles.buttonRow]}>
          <ActionButton
            onPress={() => navigation.navigate('EditSplit', { data: split })}
            height={'large'}
            text={'Edit'}
            width={'small'}
        ...
        <Text style={styles.description}>
          Note: If you make changes to a split made by someone else, you will be
          unsubscribed from their split
        </Text>
      </ScrollView>
      <CodeBanner
        label={'Split Share Code'}
        code={generateSplitShareCode(split.id, split.creator.id)}
        data={{}}
      />
    </View>
  );
