      ...
      <ScrollView
        ...
        <InputWrapper label={'Split'} valid={validation.selectedSplit}>
          {Object.keys(selectedSplit).length > 0 ? (
            <SplitCard
              split={selectedSplit}
              onDelete={() => setSelectedSplit({})}
            />
          ) : (
            <ScrollView
              style={styles.yourSplits}
              nestedScrollEnabled={true}
              showsVerticalScrollIndicator={false}>
              <SearchBar
        ...
        <InputWrapper label={'Repeating?'} valid={validation.selectedDays}>
          ...
          {repeating && (
            <View>
              <View style={systemStyles.formSpacer} />
              <Counter
                formattedValue={`for ${weekCounter} weeks`}
                initialValue={weekCounter}
                min={1}
                increment={1}
                onChange={setWeekCounter}
              />
              <View style={systemStyles.formSpacer} />
                ...
                {selectedDays.map((val, ind) => (
                  <TouchableWithoutFeedback
                    ...
                    <View
                      style={[
                        styles.dayBtn,
                        val === 1 ? styles.dayBtnActive : null,
                      ]}>
