...
const msToHM = (ms, format = 'hm') => {
  ...
  return `${hours > 0 ? hours + hourSuffix + ' ' : ''}${
    minutes > 0 ? minutes + minuteSuffix + ' ' : ''
  }`;
};
...
const filterSplitsByString = (splitList, str) => {
  ...
  return Object.entries(splitList).filter(([id, split]) =>
    filterSplit(str.toLowerCase(), split),
  );
};
/**
...
const getSplitFromShareCode = async shareCode => {
  ...
  const split = await firebaseFunc
    .functions()
    .httpsCallable('getSplitFromUser')(decoded);
  return split;
};

const getVerifiedSplits = async () => {
  const res = await firebaseFunc
    .functions()
    .httpsCallable('getVerifiedSplits')();
  return res.data;
};
...
const getVerifiedMovements = async () => {
  const res = await firebaseFunc
    .functions()
    .httpsCallable('getVerifiedMovements')();
  return res.data;
};
