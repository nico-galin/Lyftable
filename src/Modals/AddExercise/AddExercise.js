...
export const AddExercise = ({ isVisible, setVisibility }) => {
  ...
  const filteredMovements = Object.fromEntries(Object.entries(verifiedMovements).filter(([key, value]) => 
    value.name.toLowerCase().includes(movementSearchText.toLowerCase()) || (value.alt_name && value.alt_name.toLowerCase().includes(movementSearchText.toLowerCase()))
  ));
  return (
    <ModalContainer isVisible={isVisible} setVisibility={setVisibility} header={'Add Exercise'}>
        <InputWrapper label={'Movement'} valid={validation.exercise}>
          ...
          :
            <View>
              <SearchBar value={movementSearchText} placeholder={'Search Verified Movements...'} onChangeText={setMovementSearchText} />
        ...
        <InputWrapper label={'Time Limit'}>
          <Counter formattedValue={timeLimit === 0 ? "None" : msToHM(timeLimit)} initialValue={timeLimit} increment={60000} min={0} onChange={setTimeLimit}/>
        </InputWrapper>
        <InputWrapper label={'Set Count'}>
          <Counter formattedValue={setCount} initialValue={setCount} increment={1} min={1} onChange={handleChangeSets}/>
        </InputWrapper>
        <InputWrapper label={'Repetition Count'}>
          <OptionSlider options={["Equal Reps", "Different Reps"]} defaultIndex={0} onChange={handleChangeEqualReps}/>
          {equalReps ?
        ...
        <InputWrapper label={'Rest Time'}>
          <Counter formattedValue={restTime === 0 ? "None" : msToHMS(restTime)} initialValue={restTime} increment={15000} min={0} onChange={setRestTime}/>
        </InputWrapper>
        <View style={[styles.rep, {width: '100%'}]}>
          <ActionButton onPress={completeForm} text={'Done'} height={'large'} color={theme.SECONDARY_COLOR} textColor={theme.BACKGROUND_COLOR} />
        </View>
