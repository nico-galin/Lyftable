...
export default () => {
  ...
  for (let i = 0, l = 42; i < l; i++) {
    ...
    dateList.push({
      ...
      type: ['plain', 'missed', 'scheduled', 'completed'][Math.floor(Math.random() * (4))],
      ...
      <View style={styles.monthCarousel}>
          <TouchableWithoutFeedback onPress={() => setMonthDiff(monthDiff - 1)}>
            <View style={styles.monthCarouselLeftBtn}>
              <Icon name={'chevron-left'} size={20} color={theme.SUBTITLE_COLOR}/>
            </View>
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback onPress={() => setMonthDiff(0)}>
            <Text style={styles.monthText}>{isSameYear(referenceDate, today) ? format(referenceDate, 'MMMM') : format(referenceDate, 'MMMM yyyy')}</Text>
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback onPress={() => setMonthDiff(monthDiff + 1)}>
            <View style={styles.monthCarouselRightBtn}>
              <Icon name={'chevron-right'} size={20} color={theme.SUBTITLE_COLOR}/>
            </View>
          </TouchableWithoutFeedback>
      </View>
      ...
      <View style={styles.footer}>
        <View style={styles.key}>
          <View style={styles.keyElement}>
            <View style={styles.missedCircle}></View>
            <Text style={styles.keyText}>Missed</Text>
          </View>
          <View style={styles.keyElement}>
          <View style={styles.scheduledCircle}></View>
